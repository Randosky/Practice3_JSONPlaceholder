...
class FakeJSONPlaceholderApi {

    private val postUiModels: List<PostUiModel> = listOf(
        PostUiModel(
            userId = 1,
            id = 1,
    ...
    private val commentUiModels: List<CommentUiModel> = listOf(
        CommentUiModel(
            postId = 1,
            id = 1,
    ...
    private val userUiModels: List<UserUiModel> = listOf(
        UserUiModel(
            id = 1,
            name = "Leanne Graham",
            userName = "Bret",
    ...
    fun getPosts(): List<PostUiModel> {
        return postUiModels;
    }

    /** Получаем комментарии по id поста */
    fun getCommentsByPost(postId: Int): List<CommentUiModel> {
        return commentUiModels.filter { it.postId == postId }
    }
